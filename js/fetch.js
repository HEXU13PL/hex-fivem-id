...
export const renderPlayers = (players, search = false) => {
    ...
    players.forEach((player) => {
        ...
        if (player.socials && player.socials.discord) {
            ...
            discordContainer.style.cssText = 'display: inline-flex; align-items: center; gap: 8px; margin-left: 10px; font-size: 0.95em; color: #5865F2; background: rgba(88, 101, 242, 0.15); padding: 4px 12px; border-radius: 12px; vertical-align: middle; cursor: pointer; user-select: none; transition: background 0.2s;';
